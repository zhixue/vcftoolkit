...
"""
    ...
    @Modified: 2022/12/1 1:14 PM
    @Usage:
"""
# python3 leftjoin_nofixed_domain.py file1 file1_col file2 file2_col output
import sys
...
with open(sys.argv[3]) as f:
    for line in f:
        ...
        if temp[int(sys.argv[4])-1] not in f2_dict:
            f2_dict[temp[int(sys.argv[4])-1]] = ['::'.join(line.rstrip().split('\t')[4:6])]
        else:
            if '::'.join(line.rstrip().split('\t')[4:6]) not in f2_dict[temp[int(sys.argv[4])-1]]:
                f2_dict[temp[int(sys.argv[4]) - 1]] += ['::'.join(line.rstrip().split('\t')[4:6])]
